...
- [checklists/requirements.md](checklists/requirements.md) (created/updated by `/sdd.specify`)
...
## Change Log *(optional — populate when spec is incrementally updated via `/sdd.prd2spec --base`)*

<!--
  Record spec-level changes here for traceability. Each row corresponds to a modification of an existing item.
  Managed by /sdd.prd2spec incremental mode (--base flag). Leave this section absent when using /sdd.specify.
-->
