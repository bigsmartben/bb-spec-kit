...
<!--
  ...
  - 本索引只给“一句话能力声明 + 引用场景 + 链接导航”，不重复长描述；
  - 3.3 中每条 FR-### 应包含最小 Given/When/Then 摘要与 UDD refs（`Entity.field`），用于验收与口径追溯。
-->
...
<!--
  ...
  ✅ 具象化要求（本节是“可实现/可测试”的关键）：
  - 每条 FR 必须包含一个“可观察的行为断言”（Given/When/Then 的最小摘要即可），确保测试可落地。
  - 每条 FR 必须显式列出关联的 UDD Items（`Entity.field`）：
    - Reads/Displays：此 FR 会读取并呈现给用户的 UDD items
    - Writes/Updates：此 FR 会导致口径变化/新增的用户可见信息（若有，必须回填到 § 1.3 UDD）
  - 每条 FR 必须写清楚“成功结果 + 失败/异常结果”至少各 1 条（异常可 ref: EC-xxx）。

  ❌ 禁止：
  - 不写接口/endpoint、DB 表、队列、锁、框架、具体实现方案
  - 不用“优化/支持/提升/可配置”等不可验收词汇替代明确行为
-->
...
#### FR-002 — [Short requirement title] *(Level: MUST | ref: S1, S2)*

- **Capability**: System MUST [capability statement]
- **Given/When/Then (minimum)**:
  - **Given**: [...]
  - **When**: [...]
  - **Then**: [...]
- **UDD (user-visible data) refs**:
  ...
  - **Writes/Updates**: [None] / `Entity.field`
- **Success criteria (testable)**: [...]
- **Failure / edge behavior**: [...]

#### FR-003 — [Short requirement title] *(Level: SHOULD | ref: S2)*

- **Capability**: System SHOULD [capability statement: async or non-core behavior]
- **Given/When/Then (minimum)**:
  - **Given**: [...]
  - **When**: [...]
  - **Then**: [...]
...
#### FR-004 — [Short requirement title] *(Level: MUST | ref: Sx)*

- **Capability**: System MUST [data/record/business rule requirement — still user-observable]
- **Given/When/Then (minimum)**:
  - **Given**: [...]
  - **When**: [...]
  - **Then**: [...]
...
#### FR-005 — [Short requirement title] *(Level: MUST | ref: Sx)*

- **Capability**: System MUST [NEEDS CLARIFICATION: rule not defined, e.g., max per user? max per day?]
- **UDD (user-visible data) refs**:
  - **Reads/Displays**: [None] / `Entity.field`
  - **Writes/Updates**: [None] / `Entity.field`
