...
<!--
  ...
  3.1 用户故事 & 验收 → 3.2 交互流程 → 3.3 功能需求 → 3.4 UI 定义

  读者在此一节内可获取该 UC 从业务意图到实现契约的所有信息，无需跳转其他章节。
  可选小节（3.2 / 3.4）按实际情况决定是否填写，不需要时直接删除。
-->
...
### 3.2  用户交互流程 *(optional — include when UC involves multi-step or branching interactions)*
