...
2. **Check checklists status** (if FEATURE_DIR/checklists/ exists):
   ...
   - Classify checklist gate type:
     - **Advisory (soft gate)**: `udd-vo-coverage.md` (generated by `/sdd.tasks`)
...
3. Load and validate the implementation context (fail fast):
   ...
   - **REQUIRED (for HTTP API features with `operationId`)**: Prefer loading `contracts/interface-details/<operationId>.md` for each active `IFxx` as the primary interface design source
     - If an expected interface detail doc is missing: **ERROR** and STOP (rerun `/sdd.tasks`)
   ...
   - **OPTIONAL**: `research.md`
     ...
     - Do NOT create new design artifacts during `sdd.implement` unless explicitly required by `tasks.md`
...
5. Parse `tasks.md` into an execution model (Interface-scoped + DAG-driven):
   ...
   - **REQUIRED**: Extract **Task DAG (Adjacency List)** and build a dependency graph
     - If the adjacency list is missing/unparseable: **ERROR** and STOP (rerun `/sdd.tasks`)
...
- For verification tasks (`Type:Test`):
  ...
  - If `contracts/test-case-matrix.md` exists:
    ...
    - If the task references `CaseID`s that cannot be found in the matrix: **ERROR** and STOP (regenerate via `/sdd.plan` or `/sdd.tasks`)
