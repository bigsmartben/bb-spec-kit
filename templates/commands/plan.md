...
handoffs: 
  ...
  - label: Create Checklist
    ...
    send: true
scripts:
  sh: scripts/bash/setup-plan.sh --json --input "{ARGS}"
  ps: scripts/powershell/setup-plan.ps1 -Json -InputFile "{ARGS}"
agent_scripts:
  sh: scripts/bash/update-agent-context.sh __AGENT__ --input "{ARGS}"
  ps: scripts/powershell/update-agent-context.ps1 -AgentType __AGENT__ -InputFile "{ARGS}"
---
...
- `/sdd.plan <spec.md> [notes...]`
- `<spec.md>` can be an absolute path or a repo-root relative path under `specs/<feature>/`.

Examples:

- `/sdd.plan specs/001-foo/spec.md`
- `/sdd.plan /abs/path/to/specs/001-foo/spec.md 请优先关注稳定性约束`
...
- Contract file: `scripts/contracts/setup-plan.json`
- Agent context update contract: `scripts/contracts/update-agent-context.json`
- Treat these contracts as authoritative for script I/O and side effects.
...
- `plan.md` 的结构与固定标签以模板为准：
  - Preferred: `.specify/templates/plan-template.md`
  - Fallback: `templates/plan-template.md`
- 本命令负责阶段编排、证据收集、产物生成与 gate 执行；不应重复发明模板结构。
- 若命令文本与模板结构冲突，优先以模板为准，并将该冲突视为需修复的漂移问题。

## Outline

1. **Setup**: Run `{SCRIPT}` from repo root and parse JSON for `FEATURE_SPEC`, `IMPL_PLAN`, `SPECS_DIR`, `FEATURE_DIR`, `INPUT_FILE_ABS`, `BRANCH`. Treat `INPUT_FILE_ABS` as the context root. For single quotes in args like "I'm Groot", use escape syntax: e.g 'I'\''m Groot' (or double-quote if possible: "I'm Groot").
...
3. **Execute plan workflow**: Follow the structure in IMPL_PLAN template to:
   ...
   - Phase 1: Generate design artifacts in strict dependency order:
     ...
     - `quickstart.md`
   - NOTE: Per-operation interface detail docs under `contracts/interface-details/` are generated by `/sdd.tasks` (not `/sdd.plan`) when `contracts/openapi.yaml` is present.
   - Phase 1: Update agent context by running the agent script
   - Re-evaluate Constitution Check post-design

4. **Stop and report**: Command ends after Phase 2 planning. Report branch, IMPL_PLAN path, and generated artifacts.
  - Recommend running `/sdd.preview` to generate/update reviewer-facing `preview.html`.
...
4. **Interface Detail Docs (per operation)** *(frontend ↔ backend HTTP API features only)*:
   - Do NOT generate per-operation interface detail docs during `/sdd.plan`.
   - These docs are generated by `/sdd.tasks` from `contracts/openapi.yaml` and are written to:
