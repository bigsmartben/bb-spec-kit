...
.PARAMETER Agents
    ...
    Valid agents: claude, gemini, copilot, cursor-agent, qwen, opencode, windsurf, codex, kilocode, auggie, roo, codebuddy, cline, amp, q, bob, qodercli, shai, agy, generic
...
& $pythonCmd ".github/workflows/scripts/check-agent-metadata-consistency.py"
...
function Build-Variant {
    ...
    switch ($Agent) {
        ...
        'cline' {
            $cmdDir = Join-Path $baseDir ".cline/workflows"
            Generate-Commands -Agent 'cline' -Extension 'prompt.md' -ArgFormat '$ARGUMENTS' -OutputDir $cmdDir -ScriptVariant $Script
        }
        'amp' {
...
$AllAgents = @('claude', 'gemini', 'copilot', 'cursor-agent', 'qwen', 'opencode', 'windsurf', 'codex', 'kilocode', 'auggie', 'roo', 'codebuddy', 'cline', 'amp', 'q', 'bob', 'qodercli', 'shai', 'agy', 'generic')
$AllScripts = @('sh', 'ps')
